...
class TextProcessor:
    ...
    def process_document_text(self, document: Document) -> List[Match]:
        ...
        """
        matches = []
        processed_patterns = set()  # Track processed patterns to avoid duplicates
    ...
    def _process_paragraph(self, paragraph: Paragraph, location: str, processed_patterns: set = None) -> List[Match]:
        ...
        for pattern_name, matched_text, start_pos, end_pos in pattern_matches:
            try:
                ...
                # Check for duplicates - skip if this pattern has already been processed
                if processed_patterns is not None:
                    pattern_key = f"{matched_text}->{replacement_text}"
                    if pattern_key in processed_patterns:
                        logger.info(f"TEXT DEDUPLICATION: Skipping already processed pattern '{matched_text}' at {location}")
                        continue
        ...
        try:
            ...
            for section_idx, section in enumerate(document.sections):
                ...
                if section.header:
                    ...
                    header_matches = self._process_paragraphs(section.header.paragraphs, f"header_section_{section_idx}",
                        processed_patterns
                    , processed_patterns)
                    matches.extend(header_matches)
                    
                    # Process header tables
                    header_table_matches = self._process_tables(section.header.tables, f"header_section_{section_idx}",
                        processed_patterns
                    , processed_patterns)
                ...
                if hasattr(section, 'first_page_header') and section.first_page_header:
                    # Process first page header paragraphs
                    first_header_matches = self._process_paragraphs(section.first_page_header.paragraphs, f"first_page_header_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(first_header_matches)
                    
                    # Process first page header tables
                    first_header_table_matches = self._process_tables(section.first_page_header.tables, f"first_page_header_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(first_header_table_matches)
                
                # Even page header
                if hasattr(section, 'even_page_header') and section.even_page_header:
                    # Process even page header paragraphs
                    even_header_matches = self._process_paragraphs(section.even_page_header.paragraphs, f"even_page_header_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(even_header_matches)
                    
                    # Process even page header tables
                    even_header_table_matches = self._process_tables(section.even_page_header.tables, f"even_page_header_section_{section_idx}"
                    , processed_patterns)
                ...
                if section.footer:
                    ...
                    footer_matches = self._process_paragraphs(section.footer.paragraphs, f"footer_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(footer_matches)
                    
                    # Process footer tables
                    footer_table_matches = self._process_tables(section.footer.tables, f"footer_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(footer_table_matches)
                
                # First page footer
                if hasattr(section, 'first_page_footer') and section.first_page_footer:
                    # Process first page footer paragraphs
                    first_footer_matches = self._process_paragraphs(section.first_page_footer.paragraphs, f"first_page_footer_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(first_footer_matches)
                    
                    # Process first page footer tables
                    first_footer_table_matches = self._process_tables(section.first_page_footer.tables, f"first_page_footer_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(first_footer_table_matches)
                
                # Even page footer
                if hasattr(section, 'even_page_footer') and section.even_page_footer:
                    # Process even page footer paragraphs
                    even_footer_matches = self._process_paragraphs(section.even_page_footer.paragraphs, f"even_page_footer_section_{section_idx}"
                    , processed_patterns)
                    matches.extend(even_footer_matches)
                    
                    # Process even page footer tables
                    even_footer_table_matches = self._process_tables(section.even_page_footer.tables, f"even_page_footer_section_{section_idx}"
                    , processed_patterns)
