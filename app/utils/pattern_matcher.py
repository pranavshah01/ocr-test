...
class PatternMatcher:
    ...
    def _compile_patterns(self):
        ...
        patterns_to_compile = self.enhanced_patterns if self.enhanced_mode else self.patterns
        
        for name, pattern in patterns_to_compile.items():
            if not name.startswith('_'):  # Skip metadata
                try:
                    self.compiled_patterns[name] = re.compile(pattern, re.IGNORECASE | re.UNICODE)
                    logger.debug(f"Successfully compiled pattern '{name}': {pattern}")
                except re.error as e:
                    logger.error(f"Invalid regex pattern '{name}': {e}")
                    # Fallback to original pattern
                    if self.enhanced_mode:
                        try:
                            original_pattern = self.patterns[name]
                            self.compiled_patterns[name] = re.compile(original_pattern, re.IGNORECASE | re.UNICODE)
                            logger.warning(f"Using original pattern for '{name}': {original_pattern}")
