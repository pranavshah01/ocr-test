...
class PreciseTextReplacer:
    ...
    def _apply_hybrid_replace(self, cv_image: np.ndarray, match: OCRMatch, all_ocr_results: List[OCRResult]) -> np.ndarray:
        ...
        try:
            ...
            # Replace the matched pattern with replacement text
            return self._draw_replaced_text(cv_image, bbox, original_full_text, matched_pattern, replacement_text, all_ocr_results)
    ...
    def _draw_replaced_text(self, cv_image: np.ndarray, bbox: Tuple[int, int, int, int],
        ...
        try:
            ...
            # Draw replacement text with background clearing
