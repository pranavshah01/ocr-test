"""
Cleanup utility for the Document Processing Pipeline.
Removes logs, reports, and __pycache__ directories to free up space.

Usage:
    python3 -m app.cleanup [--dry-run] [--logs] [--reports] [--cache] [--all]
"""
...
def cleanup_pycache_directories(start_path: Path, dry_run: bool = False) -> tuple[int, int]:
    ...
    Returns:
        Tuple of (directories_removed, total_size_freed)
    """
...
def cleanup_reports(reports_dir: Path, dry_run: bool = False) -> tuple[int, int]:
    ...
    # Find all report files (JSON and HTML)
...
def create_cleanup_argument_parser() -> argparse.ArgumentParser:
    """
    Create argument parser for cleanup command.
...
def main():
    ...
    try:
        ...
        # Get default configuration for paths
