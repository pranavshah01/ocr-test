...
class GraphicsProcessor(BaseProcessor):
    ...
    def _update_processing_result(self, processing_result: ProcessingResult, detections: List[Dict[str, Any]], reconstructions: List[Dict[str, Any]] = None) -> None:
        ...
        # Start numbering after any existing details to keep a single continuous sequence
        start_sr = len(processing_result.match_details) + 1
            ...
            if reconstruction.get('success', False):
                ...
                for match_detail in processing_result.match_details:
                    if (match_detail.orig_id_name == reconstruction.get('location', '') and
                        ...
                        # Show only the mapped 4022 text in reports (not the appended original text)
                        if 'replacement_text' in reconstruction:
                            match_detail.mapped_text = reconstruction['replacement_text']
